...
describe('queryAnalytics', () => {
  it('aggregates totals from impression buckets for the requested range', async () => {
    const r = await queryAnalytics(db, { cardId, range: '24h', now: NOW });
    expect(r.totals.totalImpressions).toBe(15);
    expect(r.totals.uniqueVisits).toBe(6);
    expect(r.series.length).toBeGreaterThan(0);
  });

  it('returns wider totals for range=all', async () => {
    const r = await queryAnalytics(db, { cardId, range: 'all', now: NOW });
    expect(r.totals.totalImpressions).toBe(17);
  });

  it('scopes to the requested cardId', async () => {
    const r = await queryAnalytics(db, { cardId: otherCardId, range: 'all', now: NOW });
    expect(r.totals.totalImpressions).toBe(99);
  });

  it('produces referrer + country breakdowns for the visits range', async () => {
    const r = await queryAnalytics(db, { cardId, range: '24h', now: NOW });
  ...
  it('excludes visits older than the range window', async () => {
    const r = await queryAnalytics(db, { cardId, range: '24h', now: NOW });
    expect(r.referrers.find((x) => x.host === null)).toBeUndefined();
  });

  it('returns a 7x24 heatmap with the visit buckets populated', async () => {
    const r = await queryAnalytics(db, { cardId, range: 'all', now: NOW });
  ...
  it('surfaces direct/camo impression split alongside totals', async () => {
    const r = await queryAnalytics(db, { cardId, range: '24h', now: NOW });
    expect(r.totals.totalImpressions).toBe(15);
  ...
  it('leaves the split at 0 for legacy buckets that predate the new columns', async () => {
    const r = await queryAnalytics(db, { cardId, range: 'all', now: NOW });
    expect(r.totals.totalImpressions).toBe(17);
    expect(r.totals.directImpressions).toBe(8);
    expect(r.totals.camoImpressions).toBe(7);
  ...
  it('returns a unique-visit source breakdown grouped by viaCamo', async () => {
    const r = await queryAnalytics(db, { cardId, range: '24h', now: NOW });
